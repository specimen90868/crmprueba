    ...
    <div id="content">
      <div id="contentmid">
		...
		$tabla="<table id='' class='recordList'>
			...
			<tbody>";
		$sqlproceso = "SELECT `oportunidades`.`clave_oportunidad`,`oportunidades`.`clave_organizacion`,`oportunidades`.`id_rechazo`,`oportunidades`.`id_etapa`,`oportunidades`.`usuario`,`oportunidades`.`fecha_captura`,`organizaciones`.`organizacion`,`motivosrechazo`.`motivo_rechazo`,`contactos`.`nombre_completo` FROM `oportunidades` JOIN (`organizaciones`, `motivosrechazo`,`contactos`) ON (`organizaciones`.clave_organizacion=`oportunidades`.clave_organizacion AND `motivosrechazo`.id_motivorechazo=`oportunidades`.id_rechazo AND `contactos`.clave_organizacion=`oportunidades`.clave_organizacion) WHERE `oportunidades`.`id_rechazo`='6' OR `oportunidades`.`id_rechazo`='7' OR `oportunidades`.`id_rechazo`='8' OR `oportunidades`.`id_rechazo`='9' ORDER by `oportunidades`.fecha_captura DESC";
		$rsproceso = mysql_query($sqlproceso,$db);	
		$i=1;
		while($rwproceso = mysql_fetch_array($rsproceso))
		{
			$tabla.="<tr class='even-row'><td class='list-column-right'>".$i."</td><td class='list-column-left'>".$rwproceso[organizacion]."</td><td class='list-column-left'><a class='keytext' href='../organizaciones/detalles.php?organizacion=".$rworg[clave_organizacion]." style='text-transform:uppercase;'>".$rworg[organizacion]."</a></td></tr>";
			$i++;
		}
		$tabla.="</tbody>
			</table>";
        ...
        <div id="resultadoorg">
        <fieldset class="fieldsetgde">
        <legend>Procesos por Etapa</legend>
